...
layoutNarrow codeAt n card mkGroups = do
    ...
    parts <- forkJoinResults [histFirstChunk codeAt card lo hi | (lo, hi) <- chunks]
...
placeBucketOffsPar shift card bucketStart offs pb visM b = do
    let !s = VU.unsafeIndex bucketStart b
        ...
        !range = min (1 `unsafeShiftL` shift) (card - base)
        packed = packedVec pb
        subChunks = [(s + lo, s + hi) | (lo, hi) <- splitRange capabilities (e - s)]
    hists <-
        forkJoinResults
...
overBuckets bucketStart n small big = do
    ...
    next <- newIORef 0
    let worker = do
            b <- atomicModifyIORef' next (\j -> (j + 1, j))
            when (b < nBuckets) $ do
                when (size b < bigCut) (small b)
                worker
    _ <- forkJoinResults (replicate (min capabilities (max 1 nBuckets)) worker)
    pure ()
